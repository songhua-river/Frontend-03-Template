    ...
    if (selector.charAt(0) === '#') {
        ...
        if (attr) {
            ...
            for (let classValue of classes) {
                if (classValue === selector.replace('.', '')) {
                    return true
                }
            }
            return false
...
function specificity(selector, p) {
    let selectorParts = selector.split(' ')
    for (let part of selectorParts) {
        // 只处理 a.a a#a .a.b .a#b 类似的复合属性
        if (selector.split(/([\.#])/).length > 2) {
            const selector2 = selector.split(/([\.#])/).filter(item => !!item).join(' ');
            p = specificity(selector2, p)
        }
...
function computeCSS(element) {
    ...
    for (let rule of rules) {
        ...
        let selectorPart = selectorParts[0]
        // 先匹配了当前元素，所以后面j为1
        if (!match(element, selectorPart)) {
            continue
        }
        ...
        if (matched) {
            let sp = specificity(rule.selectors[0], [0, 0, 0, 0,])
